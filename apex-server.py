...
<div class="steps">Each client gets: ✨ rebranded share link · 📱 Shadowrocket base64 ·
🔄 Clash auto-update link · 🔗 live quota link · 📋 ready-to-send message</div>
...
<div class="me" style="margin-top:14px"><div class="t">✨ <b>VPN link for your app</b> (v2rayNG / NekoBox / Hiddify) — subscription named <b>{html.escape(brand)}</b>, clean English node names:</div>
...
<div class="me" style="margin-top:14px"><div class="t">🔄 <b>Clash auto-update</b> (Verge / Fugu / mihomo): Profiles → New → remote URL → paste this. Updates nodes &amp; quota automatically, forever:</div>
...
<div class="me" style="margin-top:14px"><div class="t">📱 <b>Shadowrocket (easiest):</b> add this link — it arrives already named <b>{html.escape(brand)}</b>, nothing to rename:</div>
<div class="u" id="sr">{html.escape(sr_url)}</div></div>
<button class="btn green" style="margin-top:8px" onclick="cp('sr','copsr')">📋 Copy Shadowrocket link</button>
<div class="ok" id="copsr" style="display:none">✅ copied — in Shadowrocket: ➕ → Subscribe → paste → done</div>
...
<div class="err">❌ {html.escape(r["error"])}</div></div>""")
        ...
        msg = (f"Here's your {brand} setup 🚀\n\n"
               f"1) Shadowrocket (easiest) — add this link, it arrives already named \"{brand}\":\n{sr}\n\n"
               f"   Other apps (v2rayNG / NekoBox / Hiddify) — add this link:\n{share}\n"
               f"   Clash (Verge/Fugu) — use this instead, it auto-updates:\n{sub}\n\n"
...
{stat}
<div class="me"><div class="t">📱 <b>Shadowrocket</b> — arrives named <b>{html.escape(brand)}</b> automatically:</div><div class="u" id="sr{n}">{html.escape(sr)}</div></div>
<div class="me"><div class="t">✨ <b>Rebranded share link</b> (v2rayNG / NekoBox / Hiddify):</div><div class="u" id="sp{n}">{html.escape(share)}</div></div>
<div class="me"><div class="t">🔄 <b>Clash auto-update</b> link:</div><div class="u" id="su{n}">{html.escape(sub)}</div></div>
<div class="me"><div class="t">🔗 <b>Client's private quota</b> link:</div><div class="u" id="pl{n}">{html.escape(full)}</div></div>
<div style="display:flex;gap:8px">
<button class="btn green" style="flex:1;margin-top:8px" onclick="cp('sr{n}','cosp{n}')">📋 Shadowrocket</button>
<button class="btn gray" style="flex:1;margin-top:8px" onclick="cp('su{n}','cosp2{n}')">📋 Clash</button>
</div>
<div class="ok" id="cosp{n}" style="display:none">✅ shadowrocket link copied</div>
...
<div class="err">❌ {html.escape(r["error"])}</div></div>""")
        ...
        parts.append(f"""<div class="row"><h2>Link {n}</h2>
{stat}
<div class="me"><div class="t">📱 <b>Shadowrocket</b> — add this link, it arrives named <b>{html.escape(brand)}</b> automatically (nothing to rename):</div>
<div class="u" id="sr{n}">{html.escape(sr)}</div></div>
<div class="me"><div class="t">✨ <b>Rebranded link</b> — v2rayNG / NekoBox / Hiddify:</div>
<div class="u" id="sp{n}">{html.escape(share)}</div></div>
<div style="display:flex;gap:8px">
<button class="btn green" style="flex:1;margin-top:8px" onclick="cp('sr{n}','cosp{n}')">📋 Copy SR link</button>
<button class="btn gray" style="flex:1;margin-top:8px" onclick="cp('sp{n}','cossr{n}')">📋 Copy all-apps</button>
</div>
<div class="ok" id="cosp{n}" style="display:none">✅ copied</div>
<div class="ok" id="cossr{n}" style="display:none">✅ copied</div>
<a class="btn gray" href="{dl}">⬇️ Download rebranded subscription (.txt)</a>
<div class="note">📱 The Shadowrocket link above carries the name <b>{html.escape(brand)}</b> inside it — clients add it and it shows up correctly named, no renaming needed. The .txt download is the offline backup (paste into Shadowrocket → same name).</div></div>""")
